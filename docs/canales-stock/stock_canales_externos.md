...
>
> Estado global: **Fase 0 completa. Fase 1 (sin granel) APLICADA Y VERIFICADA en BD** (074–076 aplicadas por el usuario; `stock_canales_fase1_verificacion.sql` T1–T12 OK). Pendiente opcional: prueba manual de concurrencia (2 sesiones). **Fase 1b (granel) APLICADA Y VERIFICADA en BD** (077–078; `stock_canales_fase1b_verificacion.sql` G1–G17 OK). Pendiente: desplegar la app (DESPUÉS de las migraciones) y concurrencia manual G9. **Fase 2 (núcleo de canales) APLICADA Y VERIFICADA en BD** (079; `stock_canales_fase2_verificacion.sql` K1–K7 OK). **Fase 3 (aceptación automática) APLICADA Y VERIFICADA en BD** (080–081; `stock_canales_fase3_verificacion.sql` L1–L7 OK). Siguiente: Fase 4.
