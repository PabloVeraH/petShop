...
>
> Estado global: **Fase 0 completa. Fase 1 (sin granel) APLICADA Y VERIFICADA en BD** (074–076 aplicadas por el usuario; `stock_canales_fase1_verificacion.sql` T1–T12 OK). Pendiente opcional: prueba manual de concurrencia (2 sesiones). **Fase 1b (granel) APLICADA Y VERIFICADA en BD** (077–078; `stock_canales_fase1b_verificacion.sql` G1–G17 OK). Pendiente: desplegar la app (DESPUÉS de las migraciones) y concurrencia manual G9. **Fase 2 (núcleo de canales) APLICADA Y VERIFICADA en BD** (079; `stock_canales_fase2_verificacion.sql` K1–K7 OK). **Fase 3 (aceptación automática) APLICADA Y VERIFICADA en BD** (080–081; `stock_canales_fase3_verificacion.sql` L1–L7 OK). **Fase 4 (catálogo y disponibilidad) APLICADA Y VERIFICADA en BD** (082; `stock_canales_fase4_verificacion.sql` M1–M14 OK). Pendiente: desplegar y programar `canales-reconciliar` (pg_cron, §7.1). **Fase 5 (seguridad, contabilidad y operación) APLICADA Y VERIFICADA en BD** (083; `stock_canales_fase5_verificacion.sql` N1–N4 OK). **Fase 6 (salida a producción): implementado lo que depende del código; 084 APLICADA, 085 pendiente del despliegue** — el resto depende del acceso de partner y del despliegue (`rappi_salida_produccion.md`).
...
| D24 | (2026-09-25) Contabilización de la liquidación de una plataforma (al conciliar, D17): **Dr Banco (monto depositado) + Dr Comisiones Canales Externos 520101 (comisión neta de IVA) + Dr IVA Crédito Fiscal (IVA de la comisión) / Cr CxC <canal> (bruto del período)**. Supone que la plataforma factura la comisión con IVA. |
...
| V22 | (2026-09-25, documentación oficial dev-portal.rappi.com/en/webhook-events y /en/api-reference/webhooks) **Rappi registra UNA URL y UN secreto POR EVENTO** (`POST .../webhook {event, data:[{url, stores}]}`, `PUT .../webhook/{EVENT}/reset-secret`) y el cuerpo **no trae `event_type`**. `NEW_ORDER`: id en `order_detail.order_id`, ítems con `sku`, `price` = unitario sin descuento, `unit_price_with_discount`. PING responde `{"status":"OK","description":"Store on"}`. Header `Rappi-Signature: t=…,sign=…` sobre `"t.cuerpo"` (HMAC-SHA256 hex); la tolerancia anti-replay no está documentada (se usa 5 min, supuesto). El adaptador anterior asumía `event_type` en el cuerpo y `order_id` en la raíz: nunca habría reconocido una orden real. Fase 2 lo corrige con `?evento=` en la URL registrada y secreto opcional por evento. | documentado; sin sandbox |
| V23 | (2026-09-25) `canal_config.external_store_id` NULL en las 4 configuraciones; `canales_externos.habilitado = false` para rappi/pedidosya/ubereats/instagram (solo `pos` true) → con el webhook nuevo, ningún canal recibe órdenes hasta habilitarlo explícitamente (cubre 2.3b). Credenciales de Rappi guardadas con los campos anteriores (`api_key`, `api_secret`) ya no validan: hay que reingresarlas con `client_id`/`client_secret`. | verificado |
| V24 | (2026-09-25) `/api/cron/*` NO estaba en `publicRoutes` del middleware: `auth.protect()` rechaza una llamada sin sesión Clerk, así que el cron de Vercel (`email-alerts`, único programado en `vercel.json`) y cualquier llamada de pg_net no llegaban al handler (inferido por código; sin logs de Vercel). Además `email-alerts` y `audit-cleanup` aceptaban `Bearer undefined` si faltaba `CRON_SECRET`. Corregido en la Fase 3 (MW-32, I-659/I-660). | inferido |
| V25 | (2026-09-25) `/api/canales/liquidacion` no coincidía con el schema real de `canal_liquidaciones` (insertaba `periodo_inicio/periodo_fin/total_ventas/estado`; la tabla tiene `periodo_desde/periodo_hasta/monto_bruto` y no tiene `estado`) → todo POST fallaba con 500; el GET filtraba por `periodo` inexistente; sin control de rol ni asiento. Corregido en la Fase 5. | verificado |
...
- `venta_items`: ya tiene `es_granel` y `gramos`; agregar `saco_abierto_id`
  ...
  **Implementado distinto (Fase 1b, 2026-09-24):** V20 mostró que
  `venta_items` NO tenía `es_granel`/`gramos` → 077 las crea. En vez de una
  columna `saco_abierto_id` se creó `venta_item_sacos` (venta_item, saco,
  gramos), análoga a `venta_item_lotes`, porque una venta confirmada puede
  consumir el resto de un saco y parte del siguiente. `productos.stock` NO
  se derivó con una columna `stock_cerrado`: sin lotes, cerrados =
  `stock − ROUND(gramos/peso, 3)` y toda función que cambia gramos aplica
  `stock − fracción_anterior + fracción_nueva` (`ajustar_stock_por_saco`);
  con lotes, el trigger calcula `Σ lotes + fracción`. Ver encabezado de
  `migrations/077_granel_sacos_abiertos.sql` (invariantes I4/I5).
...
> **Plan gratuito de Supabase (verificado 2026-09-25 vía MCP):** `pg_cron` 1.6.4 y `pg_net` 0.20.0 están disponibles en el proyecto (no instaladas) y `supabase_vault` está instalado; la documentación no las restringe por plan. Riesgos del plan gratuito: pausa tras ~7 días de baja actividad (en pausa no corre nada), 500 MB de base (`cron.job_run_details` no se limpia solo → job de limpieza en la 085). Migración real: `migrations/085_pg_cron_canales.sql`.
...
| 2026-09-24 | Commits de Fase 1 | Plan y SQL movidos a `docs/canales-stock/` (referencias actualizadas en migraciones, tests y spec-registry). | `01c4519` (docs), `9ffdb03` (Fase 1) | Migraciones 074–076 siguen **sin aplicar**. `.mcp.json` queda fuera del repo (config local). |
| 2026-09-24 | Fase 1 — aplicación y verificación real | **Verificado**: el usuario aplicó 074 → 075 → 076 en el SQL Editor. Estado post-aplicación confirmado por consulta de solo lectura (una sola versión de cada función de stock, `devolver_stock_a_lotes` eliminada, `productos_stock_no_negativo` validado, `stock_movements.cantidad` NUMERIC). Primera corrida del script de verificación falló por un error del propio script (`worker_clerk_id = 'verif'` viola la FK `ventas_worker_clerk_id_fkey` → clerk_users); corregido a NULL. Segunda corrida: **`FASE 1: TODAS LAS VERIFICACIONES OK`** (T1–T12: stock estricto, CHECK, FIFO vigentes + aislamiento, D11/LOTE-0, NC parciales S13, anulación a lotes S12/§23.5, D2 en `crear_venta_tx`, devolución sin lotes, conteo D22, merma D23, grants). Sin datos de prueba persistidos (0 tiendas/productos/movimientos VERIF). | ver commit | Pendiente: prueba manual de concurrencia (2 sesiones, instrucciones al final del script). Datos históricos (+1 Q15, +2 V19) se corrigen por conteo físico. |
| 2026-09-24 | Fase 1b — paso previo: MCP Supabase y re-verificación | **Verificado**: el MCP estaba en modo escritura (`postgres`, `transaction_read_only = off`); `.mcp.json` pasado a `&read_only=true` (efectivo al reconectar; en esta sesión solo se ejecutaron SELECT). Re-verificado contra la BD real: S9/V12 (`crear_venta_tx` vigente conserva `(v_item->>'cantidad')::INTEGER`), V7 resuelto por 074 (`stock_movements.cantidad`, `venta_item_lotes.cantidad` NUMERIC), V16 (`nota_credito_items.cantidad_devuelta` INTEGER), V20 (`venta_items` sin `es_granel`/`gramos`), sin tabla `sacos_abiertos`. 2 productos granel, ambos con `peso_gramos` (G8 OK), ninguno con lotes, 1 con stock decimal (se corrige por conteo, D22). Funciones de 074–076 con EXECUTE solo `postgres`/`service_role`. | `6cf2ba8` (plan) | Datos históricos (+1 Q15, +2 V19): por conteo físico (D22), sin backfill. |
| 2026-09-24 | Fase 1b — granel (1.5, §4.6, D18–D20, G1–G11) | **Implementado; tests con mocks verificados; migraciones NO aplicadas.** `077_granel_sacos_abiertos.sql`: `sacos_abiertos` (índice único parcial, un saco abierto por producto, G9), `venta_item_sacos`, `venta_items.es_granel/gramos` (V20), `nota_credito_items.cantidad_devuelta` NUMERIC (V16), CHECK G8, RLS de lectura por tienda, trigger de lotes = Σ lotes + fracción, guardia de `peso_gramos` con saco abierto, `decrement_stock` y `convertir_stock_suelto_a_lote` sobre sacos cerrados, RPCs `abrir_saco` / `consumir_granel` / `devolver_granel` / `cerrar_saco_merma` / `deshacer_apertura_saco`, `ajustar_stock_conteo` con `p_gramos_saco_abierto` (firma nueva, DROP+CREATE). `078_granel_ventas_devoluciones.sql`: `crear_venta_tx` sin casts `::INTEGER` (granel por gramos, apertura forzada solo con `abrir_saco`, por unidad exige entero, `p_user_id` nuevo), `anular_venta_tx` y `crear_nota_credito_tx` devuelven gramos netos al saco (G7, §23.5 intacto), COGS proporcional (G5). App: `POST /api/productos/[id]/saco` (abrir/merma: cualquier usuario de la tienda; deshacer: solo admin), `POST /api/ventas` (gramos = verdad, 409 `REQUIERE_ABRIR_SACO`, COGS proporcional, G8), conteo con gramos, NC en kg, GET productos/inventario con `saco_abierto_gramos`, POS (confirmación forzada, "Abrí un saco nuevo", merma, "N sacos + X kg"), Inventario ("N sacos + X kg", "Deshacer apertura" admin, conteo de gramos), devolución granel de línea completa. Gates: `npm test` **202/202 suites, 2313/2313 tests**; `typecheck` limpio; `lint` sin problemas nuevos (conteo por archivo = HEAD); `npm run build` OK; sintaxis SQL/PL-pgSQL de 077, 078 y del script validada con `libpg-query`; regresión demostrada revirtiendo la ruta de ventas (8/9 fallan) y `SearchProductos` (9/10 fallan). IDs I-572..I-603, U-163..U-166, GR-U-11..21, CF-08..10, DV-21..22, DAB-01..03, IV-19 en `docs/spec-registry.md`. | `6cf2ba8` | **Pendiente**: aplicar 077 → 078 (confirmación del usuario) y correr `stock_canales_fase1b_verificacion.sql` (G1–G17) + concurrencia manual G9 (al final del script). Decisiones de implementación a confirmar: ver cierre de la sesión (G7 sin saco de origen, merma por cualquier usuario, sin asiento de merma). |
| 2026-09-24 | Fase 1b — aplicación y verificación real | **Verificado**: con confirmación explícita del usuario se aplicaron 077 → 078 vía MCP (`execute_sql`, mismo contenido de los archivos; no queda registro en la tabla de migraciones de Supabase, igual que 074–076 aplicadas por SQL Editor). Pre-chequeo: sin tablas nuevas, `cantidad_devuelta` INTEGER, 0 productos que violen G8. Post-chequeo: tablas/columnas/CHECK/trigger creados, una sola versión de `crear_venta_tx` (18 args) y `ajustar_stock_conteo` (7 args), EXECUTE solo `postgres`/`service_role`, sin casts `::INTEGER` de cantidad. Script de verificación: **`FASE 1b: TODAS LAS VERIFICACIONES OK`** (G1–G17). Sin datos de prueba persistidos (0 tiendas/productos/sacos/ventas/movimientos VERIF). | `6cf2ba8` (plan y encabezados de 077/078) | Pendiente: desplegar la app **después** de las migraciones (ya cumplido el orden); concurrencia manual G9; volver el MCP a solo lectura (reconectar `/mcp`). |
| 2026-09-24 | Fase 1b — decisiones confirmadas por el usuario | (1) **G7**: si hay saco abierto, los gramos devueltos (NC/anulación) van a ese saco; si no, se crea un saco nuevo con los gramos devueltos (origen `devolucion`) — coincide con lo implementado en `devolver_granel`. (2) Merma del saco abierto: cualquier usuario de la tienda (queda `cerrado_por`) — OK. (3) Asiento contable de merma (saco y lote) y de ajuste por conteo: **pendiente**, documentado en `docs/pendientes.md` §"Contabilidad de mermas y ajustes de stock". (4) Devolución granel de línea completa en la UI — OK. | `6cf2ba8` (plan), `3ca644d` (pendientes.md), `db980f9` (fix Jest .tsx) | Hallazgo aparte corregido: `tests/unit/components/*.test.tsx` no se ejecutaba (ver spec-registry). |
| 2026-09-25 | Fase 2 — re-verificación | **Verificado** contra código y BD: C1 (webhook no público), C2 (solo Rappi por `if`), C3 (payload), C4 (`externalStoreId ""`, `credentials {}`; `external_store_id` NULL en BD), C5 (`api_key` vs `client_id`), C16 (UNIQUE global), C17/V1 (columnas cifradas existen), C19 (default dev, buffer 24 h), V2/V3 vigentes. Nuevos: **V22** (Rappi: URL y secreto por evento, formato real del payload) y **V23** (habilitación global en false; credenciales viejas de Rappi). Next.js 16.2.4: `middleware.ts` sigue vigente (renombrado a "Proxy" sin cambio funcional, docs locales `16-proxy.md`). | `87f2c16` | — |
| 2026-09-25 | Fase 2 — núcleo común (2.1–2.9) | **Implementado; tests con mocks verificados; 079 NO aplicada.** `migrations/079_canales_nucleo.sql` (UNIQUE por tienda, columnas del ciclo de vida, CHECK de estados sin `reserved`, `canal_outbox` con dedupe de vivos, `recargo_pct`, RLS). Capas nuevas: `domain/` (tipos, máquina de estados, precio D7/D13/D14, cupo D4), `adapters/` (puerto `ChannelAdapter`, registry por `ENABLED_CHANNELS`, adaptador Rappi con Zod del payload real, cliente HTTP con URL base obligatoria en producción y caché de token coherente — C19), `infrastructure/context.ts` (`loadChannelContext`: habilitación global + por tienda, credenciales descifradas y validadas — C4), `application/recibir-evento.ts`. Webhook: ruta pública (C1), genérico vía registry (C2), `INSERT … ON CONFLICT DO NOTHING` (C16), orden normalizada (C3), PING, cancelación solo de órdenes `pending`, verificación de que el evento sea de la tienda configurada, límite de tasa de webhooks. Config: schemas desde fuente única `lib/canales/campos.ts` (C5), `external_store_id` (C4), **POST/PATCH solo admin (D8)**, PedidosYa/UberEats "Integración pendiente" (2.7, 409). UI de config con los mismos campos y aviso. Gates: `npm test` **205/205 suites, 2393/2393 tests**; `typecheck` limpio; `lint` sin problemas nuevos; `build` OK; cobertura de archivos nuevos 80–100 %; regresión demostrada (24 tests fallan con el código anterior). IDs I-604..I-625, U-167..U-176, CTR-01..14, MW-30..31, CC-17..19. | `87f2c16` | **Pendiente**: aplicar 079 (confirmación) y correr `stock_canales_fase2_verificacion.sql` (K1–K7). Fuera de esta fase: órdenes programadas (`NEW_ORDER_SCHEDULED`, hoy ignoradas — decidir en Fase 3); rutas heredadas (`hub.ts`, `registry.ts` viejo, catálogo, accept/reject manuales) se reemplazan en Fases 3–4; endpoint de token de Rappi y tolerancia anti-replay sin verificar hasta el sandbox (Fase 6). |
| 2026-09-25 | Fase 2 — aplicación y verificación real | **Verificado**: con confirmación explícita del usuario se aplicó 079 vía MCP (`execute_sql`, mismo contenido del archivo). Pre-chequeo: 0 órdenes, UNIQUE global presente, sin `canal_outbox` ni `recargo_pct`. Script de verificación: **`FASE 2: TODAS LAS VERIFICACIONES OK`** (K1–K7). Post-chequeo: constraints de `canal_ordenes` = estado_check, intentos_no_negativo, items_es_array, store_canal_external_key; 0 datos de prueba persistidos; las 4 filas de `canal_config` intactas. | `87f2c16` | Pendiente: desplegar la app después de 079 (ya cumplido el orden); volver el MCP a solo lectura (reconectar `/mcp`). |
| 2026-09-25 | Fase 3 — aceptación automática y ciclo de vida (3.1–3.10) | **Implementado; tests con mocks verificados; 080 y 081 NO aplicadas.** 3.1 `src/lib/ventas/anular-venta.ts` (extraído sin cambios de `PATCH /api/ventas/[id]`, que lo usa con el mismo contrato: ventas-detalle 18/18). 3.2 `application/procesar-orden.ts`: reclamo atómico pending→processing, SKU→producto activo de la tienda, `crear_venta_tx` con precio de la plataforma, IVA extraído, procedencia = canal, idempotency key `canal:{store}:{canal}:{orden}`; stock físico (D16) vía la BD; rechazo automático (ITEM_NOT_FOUND / ITEM_OUT_OF_STOCK) con outbox; error transitorio → reintento, 3 intentos → `failed`; asientos, auditoría y sync al Hub (C18). 3.3 webhook → `after(procesarOrden)`. 3.4 `application/outbox.ts` + `migrations/080` (`claim_canal_outbox`, SKIP LOCKED, recuperación de `processing` vencidos) + `POST/GET /api/cron/canales-outbox` (barrido de órdenes atascadas y outbox; **sin** `vercel.json`, Hobby D12; programación pg_cron pendiente de despliegue, §7.1). 3.5 `application/cancelar-orden.ts` (pending→cancelled; aceptada/lista→anular venta + cancelled — C10). 3.6 rechazo por outbox. 3.7 eliminados `hub.ts`, rutas manuales `accept`/`reject`, `POST /api/canales/orders` (C13), cron `stock-reservas-expiry` (C9) y `migrations/081` (DROP `stock_reservas`, con guard de 0 filas); queda `POST .../retry` solo admin. 3.8 `/pos/pedidos` (vista única para el storeWorker, que no accede a `/canales`), "Marcar lista" (`POST .../ready`, D8), reintento admin, aviso de pedido nuevo y enlace con contador en el POS; las 3 páginas por canal redirigen. Hallazgo **V24** (crons no públicos) corregido. Gates: `npm test` **203/203 suites, 2413/2413 tests** (DV-19 falló una vez bajo carga, pasa aislado 3/3 y en la segunda corrida completa; sin cambios en ese módulo); `typecheck` limpio; `lint` sin problemas nuevos; `build` OK; cobertura de archivos nuevos 92–100 % líneas. IDs I-626..I-660, MW-32, PCN-01..09, PCA-01..02; reemplazados I-519..I-530, CO-01..12, I-274..I-276. | `710aaac` | **Pendiente**: aplicar 080 y 081 (confirmación) + `stock_canales_fase3_verificacion.sql` (L1–L7); programar pg_cron tras desplegar (§7.1). Fuera de esta fase: `canales-reconciliar` y trabajos `availability`/`catalog` (Fase 4); órdenes programadas (`NEW_ORDER_SCHEDULED`, hoy ignoradas); alertas de outbox `dead` / órdenes `failed` (Fase 5.3); si encolar la confirmación falla tras crear la venta queda solo como `ultimo_error` (sin re-encolado automático). |
| 2026-09-25 | Fase 3 — aplicación y verificación real | **Verificado**: con confirmación explícita del usuario se aplicaron 080 → 081 vía MCP (`execute_sql`). Pre-chequeo: `stock_reservas` 0 filas y sin FK ni vistas dependientes; `claim_canal_outbox` inexistente. Script: **`FASE 3: TODAS LAS VERIFICACIONES OK`** (L1–L7: reclama solo lo listo + `processing` vencido, incrementa intentos, no re-entrega, respeta el límite, rechaza límite inválido, EXECUTE solo `postgres`/`service_role`, `stock_reservas` eliminada). 0 datos de prueba persistidos. | `710aaac` | Pendiente: desplegar la app y programar pg_cron (§7.1) tras el despliegue; concurrencia manual SKIP LOCKED (instrucciones en el script). |
| 2026-09-25 | Fase 4 — catálogo, precios y disponibilidad (4.1–4.7) | **Implementado; tests con mocks verificados; 082 NO aplicada.** 4.1 `migrations/082_canales_catalogo_disponibilidad.sql`: `precio` → `precio_override` nullable (CHECK > 0 conservado), `publicado_at`, `ultimo_disponible_publicado`, `ultima_cantidad_publicada`, `disponibilidad_publicada_at`, índice por `producto_id`; `activo` sigue siendo "habilitado en el canal". 4.2 `unidades_vendibles_canal` (con lotes: Σ lotes vigentes, D23; sin lotes: stock − saco abierto, D18; truncado), `estado_disponibilidad_canal` (fuente única: licencia D15 ∧ producto activo ∧ habilitado ∧ canal activo ∧ cupo > 0, D4; solo productos publicados), `encolar_disponibilidad_canal` y triggers `productos AFTER UPDATE OF stock, stock_minimo, activo` + `canal_producto_config AFTER UPDATE OF activo`; worker `application/disponibilidad.ts` level-triggered (lee el estado actual, envía solo cambios o todo si `completo`) y re-verificación al terminar. **Desvío del texto de §4.4**: dedupe `avail:{store}:{canal}` (no por producto) → una llamada por tienda/canal con listas turn_on/turn_off. 4.3 `GET/PUT /api/canales/[canal]/productos` (admin D8, Zod `.strict()`, ownership del producto, `logAudit`) y `recargo_pct` (0–100, 2 decimales) en `PATCH /api/canales/config`. 4.4 `canales/components/CatalogoCanal.tsx` (toggle, precio calculado, precio fijo, cupo, estado publicado, recargo, "Publicar catálogo"); las 3 páginas `*/catalogo` lo usan; enlace "Catálogo y precios" en la config del canal. 4.5 `POST /api/canales/catalog` encola `catalog` (dedupe por tienda/canal, 202); el worker (`application/catalogo.ts`) publica solo habilitados con precio §4.5, marca `publicado_at`, retira el resto y encola `avail-full`; catálogo vacío → `dead` sin llamar (borraría el menú). 4.6 `POST/GET /api/cron/canales-reconciliar` (Bearer `CRON_SECRET`, helper común `src/lib/cron-auth.ts`) encola disponibilidad completa por tienda/canal publicada, activa y desplegada. 4.7 Gates: `npm test` **206/206 suites, 2444/2444 tests**; `typecheck` limpio; `lint` sin problemas nuevos en archivos tocados (3 avisos preexistentes); `build` OK; cobertura de archivos nuevos 86–100 % líneas. IDs I-661..I-677, U-177..U-181, CTC-01..09, CC-20; I-645 actualizado (su tipo "no implementado" ya existe). | `3cd60c5` | **Pendiente**: aplicar 082 (confirmación) + `stock_canales_fase4_verificacion.sql` (M1–M14); programar `canales-reconciliar` 1 vez/día con pg_cron tras desplegar (§7.1). Riesgos: cambios solo temporales (lote que vence, licencia que vence o se renueva, canal reactivado) esperan a la reconciliación diaria; un cambio que se confirma justo mientras el worker re-verifica puede esperar a la reconciliación; dos ventas que cambian la disponibilidad de la misma tienda/canal a la vez se serializan en la dedupe_key (la segunda espera el COMMIT de la primera). `CURRENT_DATE` de la BD (UTC) vs. fecha local para vencimientos/licencia. Limpieza pendiente: `src/lib/canales/registry.ts` y adaptadores heredados sin llamadores de producción. |
| 2026-09-25 | Fase 4 — aplicación y verificación real | **Verificado**: con confirmación explícita del usuario se aplicó 082 vía MCP (`execute_sql`, mismo contenido del archivo). Pre-chequeo: `canal_producto_config` 0 filas, columna `precio` sin renombrar, ninguna función ni trigger de 082 presente; ninguna función/vista de la BD referencia la tabla. Script: **`FASE 4: TODAS LAS VERIFICACIONES OK`** (M1–M14, BEGIN … ROLLBACK). Post-chequeo: 0 residuos, `precio_override` nullable, columnas nuevas y 2 triggers presentes, 0 trabajos `availability`. | `3cd60c5` | Concurrencia manual (dos ventas sobre la misma dedupe_key, instrucciones al final del script); programar `canales-reconciliar` tras desplegar. |
| 2026-09-25 | Fase 5 — seguridad, contabilidad y operación (5.1–5.4) | **Implementado; tests con mocks verificados; 083 NO aplicada.** 5.1 `src/lib/canales/infrastructure/autorizacion.ts` (`autorizarCanales`): 401 sin sesión, 403 usuario deshabilitado (`src/lib/usuario-habilitado.ts`, fail-closed; cierra la ventana del JWT ya emitido tras el ban de Clerk), 403 no admin; aplicado a TODO `/api/canales/**` — solo `GET /orders` y `POST .../ready` quedan para cualquier usuario de la tienda (D8); Instagram, config (incluido GET), catálogo, precios, reintentos, liquidaciones y alertas: solo admin (la UI `/canales` ya era solo admin por middleware). 5.2 **D24** (decisión del usuario): `lineasLiquidacionCanal` + cuenta `COMISIONES_CANAL` 520101 + `TipoMovimiento` `LIQUIDACION_CANAL`; `POST/GET /api/canales/liquidacion` reescritas (hallazgo **V25**): Zod `.strict()` (`LiquidacionCanalSchema`), neto calculado en el servidor, período contable cerrado → 409, duplicado → 409 (UNIQUE 083), asiento `await` (acción administrativa) con compensación si falla, `journal_entry_id` vinculado, auditoría; UI `LiquidacionesCanal` en la página del canal. 5.3 `GET/POST /api/canales/alertas` (llamadas `dead` con reintento manual, reintentando, credenciales/token rechazados, pedidos `failed`, menú rechazado) + `AlertasCanales` en `/canales`; `canal_config.menu_estado` (enviado al publicar, aprobado/rechazado por webhook — antes solo `console.warn`); un 401 de Rappi invalida el token cacheado. 5.4 auditoría de rechazos automáticos, fallas definitivas, menú rechazado, liquidaciones y reintentos (config/precios/catálogo/anulaciones ya estaban). `USUARIO_SISTEMA` movido a `domain/types` (evita un import circular). Gates: `npm test` **209/209 suites, 2530/2530 tests**; `typecheck` limpio; `lint` sin errores nuevos en archivos tocados; `build` OK; cobertura de archivos nuevos 86–100 % líneas; la matriz de autorización detecta una regresión inducida (liquidación GET sin rol). IDs I-678..I-691, U-182..U-189, ALC-01..05, LQC-01..05, CC-21; I-619 actualizado. | `4631845` | **Pendiente**: aplicar 083 (confirmación) + `stock_canales_fase5_verificacion.sql` (N1–N4). Validar D24 con el contador (supuesto: la plataforma factura la comisión con IVA). Alertas solo en UI (sin email). Limpieza pendiente: adaptadores/registry heredados. |
| 2026-09-25 | Fase 5 — aplicación y verificación real | **Verificado**: con confirmación explícita del usuario se aplicó 083 vía MCP (`execute_sql`, mismo contenido del archivo). Pre-chequeo: `canal_liquidaciones` 0 filas, sin constraints de 083 ni columnas `menu_*`. Script: **`FASE 5: TODAS LAS VERIFICACIONES OK`** (N1–N4, BEGIN … ROLLBACK). Post-chequeo: 0 residuos, 3 constraints y 3 columnas presentes, `menu_estado` NULL en las 4 configuraciones. | `4631845` | Validar D24 con el contador. |
| 2026-09-25 | Fase 6 — salida a producción (6.1–6.2, parte de código) | **Implementado; tests con mocks verificados; 084 y 085 NO aplicadas.** Verificación del plan gratuito de Supabase (pregunta del usuario): `pg_cron`/`pg_net` disponibles, sin restricción por plan en la documentación; riesgos documentados (pausa por inactividad, 500 MB). 6.1: `scripts/canales/simular-rappi.mjs` (webhooks firmados como Rappi contra un preview; U-194 lo ejecuta contra un servidor local y valida con el adaptador real). 6.2: checklist verificable `GET /api/canales/[canal]/preparacion` + `PreparacionCanal` en la página del canal (despliegue, entorno sin exponer valores, habilitación global, licencia, config, credenciales, webhook recibiendo eventos, catálogo, menú, stock mínimo, crons, outbox) con las URLs del webhook por evento; `migrations/084` (`canal_config.ultimo_evento_at/tipo` —registrado por el webhook tras validar la firma— y `estado_cron_canales()`); `migrations/085` (pg_cron: outbox cada minuto, reconciliación diaria, limpieza del historial). Adaptador: `eventosWebhook` y `variablesProduccion` (opcionales en el puerto). Runbook `docs/canales-stock/rappi_salida_produccion.md`. Dato real (solo conteo): 9 de 14 productos activos con `stock_minimo` 0. Gates: `npm test` **211/211 suites, 2546/2546 tests**; `typecheck`, `lint` (archivos tocados) y `build` OK. IDs I-692..I-696, U-190..U-194, PRC-01..05, CC-22. | `61b4b69` | **Pendiente**: aplicar 084 (confirmación); desplegar; secretos en Vault + aplicar 085; acceso de partner Rappi y pruebas del runbook (§2b); definir `stock_minimo`; 6.3 PedidosYa/Uber Eats sin acceso. |
| 2026-09-25 | Fase 6 — aplicación de 084 | **Verificado**: con confirmación explícita del usuario se aplicó 084 vía MCP (`execute_sql`, mismo contenido del archivo). Pre-chequeo: sin columnas `ultimo_evento_*` ni `estado_cron_canales`; sin extensiones pg_cron/pg_net ni secretos en Vault. Post-chequeo: 2 columnas; `estado_cron_canales()` → 0 filas sin error; `anon`/`authenticated` sin EXECUTE, `service_role` con EXECUTE. **085 NO aplicada a propósito**: requiere la app desplegada y los secretos `petshop_app_url`/`petshop_cron_secret` en Vault (sin ellos pg_net fallaría cada minuto). | `61b4b69` | Desplegar → Vault → aplicar 085 (confirmación). |
