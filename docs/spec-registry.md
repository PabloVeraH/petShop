...
| I-353 | Preview reporta cogs_estimado=0 cuando no hay ventas activas en el período | GET /api/contabilidad/cierre-mes/preview | integration |
...
| I-491 | REGRESIÓN (ticket Trello 6a77e779358cdccca29dc3e3): venta pagada 100% con Nota de Crédito genera asiento de ingreso Y asiento COGS; el bloque contable se agenda con after() de next/server (no un IIFE fire-and-forget que en serverless podía quedar congelado entre ambos asientos — venta NC real quedó ~11 min con ingreso sin COGS) | POST /api/ventas | integration |
...
| I-489 | REGRESIÓN (ticket 6a76c9994e8b17f267f71641): event_type=session.ended (evento que nunca se registra en esta instancia) → 400 rechazado por validación | GET /api/user-sessions | integration |
...
| VT-07 | MEJORA (ticket Trello 6a62eb35759d896b8e127aa7): al anular con una NC activa con saldo a favor, la confirmación advierte el monto que será invalidado | SalesTicketPage | component |
| VT-08 | MEJORA (ticket Trello 6a62eb35759d896b8e127aa7): sin NC activa con saldo a favor (sin NC, NC usada, o NC de reembolso directo), la confirmación NO muestra advertencia | SalesTicketPage | component |
