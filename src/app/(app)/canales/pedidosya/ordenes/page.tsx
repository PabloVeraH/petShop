...
export default function PedidosYaOrdenesPage() {
  ...
  const fetchOrdenes = useCallback(async () => {
    try {
      const res = await fetch("/api/canales/orders?canal=pedidosya");
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="w-8 h-8 rounded-lg bg-yellow-500 flex items-center justify-center text-lg">📦</div>
        <div>
          <h1 className="text-xl font-bold text-gray-800">Órdenes PedidosYa</h1>
      ...
      {ordenes.length === 0 ? (
        ...
        <div className="space-y-3">
          {ordenes.map((orden) => (
            <div key={orden.id} className={`bg-white rounded-lg border p-4 ${orden.estado === "pending" ? "border-yellow-300 bg-yellow-50" : "border-gray-200"}`}>
              <div className="flex items-start justify-between">
                <div>
                  <p className="font-mono text-sm text-gray-600">#{orden.external_order_id}</p>
                ...
                <span className={`text-xs font-medium px-2 py-1 rounded-full ${orden.estado === "pending" ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
                  {orden.estado}
                </span>
              </div>
