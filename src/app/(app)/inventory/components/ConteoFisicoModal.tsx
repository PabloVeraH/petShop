...
export function ConteoFisicoModal({ producto, onClose }: ConteoFisicoModalProps) {
  ...
  const cantidadActual = tieneLotes ? (loteSel ? Number(loteSel.cantidad_actual) : null) : Number(producto.stock);
  ...
  const puedeEnviar = contadoValido && motivoValido && (!tieneLotes || !!loteId);
    ...
    <ModalOverlay open onClose={onClose}>
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm m-4">
        ...
        <p className="text-sm text-gray-500 mb-4">
          {producto.nombre} — stock en sistema: {producto.stock}
        </p>
        ...
        {!isLoading && !isError && (
          <div className="space-y-3">
            ...
            <div>
              <label htmlFor="conteo-cantidad" className="block text-sm font-medium text-gray-700 mb-1">Cantidad contada *</label>
              <input
                id="conteo-cantidad"
                type="number"
                min={0}
