...
export function CategoriasTab() {
  ...
  const { mutate: crearCategoria, isPending: creando } = useMutation({
    ...
    onSuccess: () => {
      ...
      setNombre(""); setDescripcion(""); setEsAlimento(false); setError("");
    },
  ...
  const { mutate: eliminarCategoria } = useMutation({
    mutationFn: (id: string) => fetch(`/api/categorias/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["categorias"] });
        ...
        {categorias.map((c) => (
          <div key={c.id} className="flex items-center justify-between px-4 py-3 border-b last:border-0">
            ...
            <div className="flex gap-2 shrink-0">
              <button onClick={() => abrirEditar(c)} className="text-xs text-blue-500 hover:underline">
                Editar
              </button>
              <button onClick={() => setConfirmDeleteId(c.id)} className="text-xs text-red-400 hover:underline">
                Eliminar
              </button>
            </div>
          </div>
