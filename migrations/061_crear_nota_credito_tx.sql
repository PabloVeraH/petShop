-- Migration 061: crear_nota_credito_tx — transacción atómica de creación de NC
--
-- Ticket Trello 6a62eb1c8b3e68c8f15f31ce ("Anular venta que tiene NC").
--
-- Investigación (julio 2026):
-- Migraciones 051-053-057 ya cerraron el lado de ANULAR venta con NC:
-- anular_venta_tx reclama atómicamente el UPDATE de estado como primera
-- operación (previene doble anulación), y los cálculos de stock/fidelización/
-- saldo consideran las NCs existentes (evita reversión duplicada).
--
-- Pero ninguna migración previa protege el CREAR la NC. La ruta POST
-- /api/notas-credito ejecuta ~9 operaciones secuenciales en JS sin
-- transacción: si cualquiera falla después de insertar la NC, queda
-- una NC "activa" con el efecto parcial aplicado — mismo defecto que
-- migrations/052 reparó retroactivamente para los casos históricos.
--
-- Esta migración envuelve la creación en una única función plpgsql que:
-- 1. Valida venta (ownership por store_id, no anulada)
-- 2. Valida cantidades disponibles contra devoluciones previas
-- 3. Calcula precios con descuento proporcional
-- 4. INSERT NC + nota_credito_items
-- 5. Restituye stock (lotes o directo) + stock_movements
-- 6. Incrementa saldo_a_favor si corresponde
-- 7. Actualiza fidelización (total_historico, descuento)
-- Cualquier error revierte TODO automáticamente.
...
BEGIN
  ...
  LOOP
    ...
    -- 2a. Leer datos del item de venta, validar pertenencia
    SELECT vi.id, vi.cantidad, vi.precio_unitario, vi.producto_id,
    ...
    -- 2d. Acumular item data para insert bulk
  ...
  INSERT INTO nota_credito_items (
    nota_credito_id, venta_item_id, producto_id,
