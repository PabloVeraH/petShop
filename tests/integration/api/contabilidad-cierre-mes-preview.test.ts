...
describe("GET /api/contabilidad/cierre-mes/preview", () => {
  ...
  describe("autenticación", () => {
    it("I-340: retorna 401 cuando no hay sesión", async () => {
    ...
    // I-351: REGRESIÓN — preview expone COGS y balance del período; sin
    // requireStoreAdmin, cualquier storeWorker autenticado podía leerlo.
    it("I-351: REGRESIÓN — retorna 403 cuando el usuario autenticado no es storeAdmin ni systemAdmin", async () => {
      mockAuth.mockResolvedValue({
  ...
  describe("validación de parámetros", () => {
    it("I-341: retorna 400 cuando falta mes", async () => {
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn().mockReturnValue(createChain({ data: [], error: null })),
      });
    ...
    it("I-341: retorna 400 cuando falta año", async () => {
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn().mockReturnValue(createChain({ data: [], error: null })),
      });
    ...
    it("I-341: retorna 400 cuando mes está fuera de rango", async () => {
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn().mockReturnValue(createChain({ data: [], error: null })),
      });
  ...
  describe("preview sin COGS", () => {
    it("I-342: retorna preview con datos del período sin crear asientos", async () => {
      let callCount = 0;
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn(() => {
          callCount++;
    ...
    it("I-344: reporta ya_tiene_cierre=true cuando el período ya está cerrado", async () => {
      let callCount = 0;
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn(() => {
          callCount++;
    ...
    it("I-352: detecta período desbalanceado", async () => {
      let callCount = 0;
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn(() => {
          callCount++;
  ...
  describe("preview no produce efectos secundarios", () => {
    it("I-345: NO llama a crearAsiento ni inserta en cierre_mes_backups", async () => {
      let callCount = 0;
  ...
  describe("fechas correctas por mes", () => {
    it("I-354: calcula febrero bisiesto (2024)", async () => {
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
    ...
    it("I-355: calcula febrero no bisiesto (2026)", async () => {
      (supabaseModule.createServiceClient as jest.Mock).mockReturnValue({
        from: jest.fn(() => createChain({ data: [], error: null })),
      });
